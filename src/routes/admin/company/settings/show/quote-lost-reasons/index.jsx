...
class QuoteLostReasons extends Component {
  state = {
    value: null,
  };
  ...
  componentDidMount() {
    ...
    retrieve(`/company_settings?company=${selectedCompany.id}&name=QUOTE_LOST_REASONS`);
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      ...
      && nextProps.retrieved['hydra:member'][0].name === 'QUOTE_LOST_REASONS'
    ) {
      return {
      ...
      <div className="section-container">
        <div className="section-general">
          <div className="option-buttons-container clearfix">
            <Header as="h3">{t('companiesQuoteLostReasons')}</Header>
            <EssorButton as={Link} to="/company/settings/quote-lost-reasons/edit" type="edit" size="tiny" floated="right">
              {t('buttonEdit')}
            </EssorButton>
          </div>
          ...
          >
            ...
            <Table celled striped>
              ...
              <Table.Body>
                {value && value.map((quoteLostReason, index) => (
                  <Table.Row key={`v${index}`}>
                    <Table.Cell>
                      {quoteLostReason.label}
                    </Table.Cell>
                  </Table.Row>
                ))}
              </Table.Body>
...
const Main = connect(mapStateToProps, mapDispatchToProps)(QuoteLostReasons);
