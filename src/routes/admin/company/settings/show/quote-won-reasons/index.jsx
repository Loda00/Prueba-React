...
class QuoteWonReasons extends Component {
  state = {
    value: null,
  };
  ...
  componentDidMount() {
    ...
    retrieve(`/company_settings?company=${selectedCompany.id}&name=QUOTE_WON_REASONS`);
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      ...
      && nextProps.retrieved['hydra:member'][0].name === 'QUOTE_WON_REASONS'
    ) {
      return {
      ...
      <div className="section-container">
        <div className="section-general">
          <div className="option-buttons-container clearfix">
            <Header as="h3">{t('companiesQuoteWonReasons')}</Header>
            <EssorButton as={Link} to="/company/settings/quote-won-reasons/edit" type="edit" size="tiny" floated="right">
              {t('buttonEdit')}
            </EssorButton>
          </div>
          ...
          >
            ...
            <Table celled striped>
              ...
              <Table.Body>
                {value && value.map((quoteWonReason, index) => (
                  <Table.Row key={`v${index}`}>
                    <Table.Cell>
                      {quoteWonReason.label}
                    </Table.Cell>
                  </Table.Row>
                ))}
              </Table.Body>
...
const Main = connect(mapStateToProps, mapDispatchToProps)(QuoteWonReasons);
